 ...
 * @param {String} to segment that needs to be extracted
 * @param {String} specObj, if not set the code will be treated as an ambigious snippet
 * @returns 
 */
 async function codeExtraction( code, from, to, specObj = { name: "VariousSnippet", type: "snippet" } ) {
	...
	// Generate it
	let resp = await cachedCompletion(fullPrompt, cachePath( specObj, "CodeExtraction" ));

	// Remove the ending "```"
	return removeSuffixIfPresent(resp, "```");
}

module.exports = codeExtraction;